...
def palette_from_svg_file(file)
  palette = {}
  svg = File.open(file) { |f| Nokogiri::XML(f) }
...
svgs = Dir["palettes/*.svg"]

svgs.each do |svg|
  palette = palette_from_svg_file(svg)
  colorscheme_name = File.basename(svg, ".*")
  params = palette.merge(name: colorscheme_name)

  templates.each do |template|
    template_name = File.basename(template, ".*")

    File.open("output/#{colorscheme_name}.#{template_name}", "w") do |file|
      file.puts(Mustache.render(File.read(template), params))
    end
  end
end
